...
(fiasco:deftest binary-split-frame-bottom-vertical-retains-total-dimensions ()
  (let ((tree:*new-split-type* :binary))
    (let ((tree (make-tree-for-tests :width 1020 :height 1023)))
      (multiple-value-bind (frame parent) (tree:split-frame-v tree :direction :bottom)
	(declare (ignore frame))
	(check-size 1023 (tree:tree-children parent) #'tree:frame-height)))))

(fiasco:deftest binary-split-frame-right-horizontal-retains-total-dimensions ()
  (let ((tree:*new-split-type* :binary))
    (let ((tree (make-tree-for-tests :width 1023 :height 1000)))
      (multiple-value-bind (frame parent) (tree:split-frame-h tree :direction :right)
	(declare (ignore frame))
	(check-size 1023 (tree:tree-children parent) #'tree:frame-width)))))

(fiasco:deftest binary-split-frame-top-vertical-retains-total-dimensions ()
  (let ((tree:*new-split-type* :binary))
    (let ((tree (make-tree-for-tests :width 1000 :height 1023)))
      (multiple-value-bind (frame parent) (tree:split-frame-v tree :direction :top)
	(declare (ignore frame))
	(check-size 1023 (tree:tree-children parent) #'tree:frame-height)))))

(fiasco:deftest binary-split-frame-left-horizontal-retains-total-dimensions ()
  (let ((tree:*new-split-type* :binary))
    (let ((tree (make-tree-for-tests :width 1023 :height 1000)))
      (multiple-value-bind (frame parent) (tree:split-frame-h tree :direction :left)
	(declare (ignore frame))
	(check-size 1023 (tree:tree-children parent) #'tree:frame-width)))))
