...
static void handle_new_layer_surface(struct wl_listener *listener, void *data) {
    ...
    struct hrt_layer_shell_surface *surface =
        hrt_layer_shell_surface_create(layer_surface, server);
...
static void check_callbacks(const struct hrt_layer_shell_callbacks *callbacks) {
...
bool hrt_layer_shell_init(struct hrt_server *server,
    ...
    server->layer_shell_callbacks = callbacks;
...
static void handle_map(struct wl_listener *listener, void *data) {
    ...
    surface->callbacks->layer_surface_mapped(surface);
}
...
static void handle_unmap(struct wl_listener *listener, void *data) {
    ...
    surface->callbacks->layer_surface_unmapped(surface);
}
...
create_popup(struct wlr_xdg_popup *wlr_popup,
    ...
    wlr_popup->base->data = popup->scene;

    // wlr_popup->base->events.configure
