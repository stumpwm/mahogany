...
hrt_scene_root_create(struct wlr_scene_tree *scene_tree) {
  assert(scene_tree != NULL);
    ...
    wl_signal_add(&scene_tree->node.events.destroy, &scene_root->listeners.scene_destroy);
