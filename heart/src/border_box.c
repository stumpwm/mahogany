...
struct hrt_border_box *hrt_border_box_create(struct wlr_scene_tree *parent,
    ...
    box->buffer = hrt_cairo_buffer_create(width, height);
    if (!box->buffer) {
    ...
    box->scene_buffer =
        wlr_scene_buffer_create(parent, &box->buffer->base);
    if (!box->scene_buffer) {
    ...
    // Sometimes the primary output isn't idenified until later,
    // so don't draw the box until we get it in the handle_outputs_update
    // callback. Maybe there's a way to ensure there's a primary output
    // here?
    if (box->scene_buffer->primary_output) {
        const float scale = box->scene_buffer->primary_output->output->scale;
        box->scale        = scale;
