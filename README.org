...
The backend library requires wlroots 0.19.x. This
version is included as a git submodule and is used by
default. See the README in the submodule or consult
the [[https://gitlab.freedesktop.org/wlroots/wlroots/-/tree/0.19.2?ref_type=tags][project's git repo]]
on how to build it.
...
like these instructions suggest. (See [[https://github.com/stumpwm/mahogany/issues/80][#80]]). Instead, just remove the submodule, and meson should be able to find the version installed on your system:

#+BEGIN_SRC bash
rm -rf heart/subprojects/wlroots/
#+END_SRC
