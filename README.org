...
milestones]]). In particular, touchscreen, menu, dialog, systray do not work
yet. But it has basic functionalities already.
...
(setf (mahogany-state-keybindings *compositor-state*)
	(list (define-kmap (kbd "s-F") #'open-firefox)
          (define-kmap (kbd "s-e") #'open-emacs)))
#+end_src

Launching in tty by =XKB_DEFAULT_OPTIONS=caps:ctrl_modifier mahagony= runs
mahogany with capslock replaced with ctrl. More options are given in
