...
In Mahogany, display screens are called outputs. Where an output is
located as well as its scale, resolution and refresh rate are
configured through the use of /output configurations/ and /output
layouts/. Output configurations apply to individual outputs, while
output layouts can apply to one or more outputs at the same time.
...
When there are multiple configurations that could match the same
output, the one with the most specificity is chosen. The order of
precedence for outputs are =:name=, =:make=, =model=, and =serial=. A
configuration with a property with higher precedence is picked over
ones with less precedence, even if more properties match. For example,
if a =serial= property matches, it will always have precedence over a
...
#+BEGIN_SRC lisp
  ...
  (mh::define-output-layout "desktop"
    ;; An external monitor is connected that is 1080p:
    ((:name "DP-1" :make "HP")
     (:position 0 0))
...
="theater"= configuration is applied. If an output with make of "HP" is
...
*** Matching configuration lenghts

If there are more connected outputs than there is in an output layout,
that layout is not considered a valid canidated unless the `exact`
flag is unset in the configuration. For example, if there are three
outputs connected and you have an output layout that contains only 2
outputs, but both of them match, that layout will not match by
default, but can be made to match if the `exact` flag is `nil`.
...
#+BEGIN_SRC text
  ...
  NAME-OR-OPTIONS : name | (name &key (priority 0) (exact t))
...
When multiple layouts could be applied with the current set of connected
...
To switch between different output layout configurations, there are
several commands available:

+ =output-layout-rescan= :: This looks up the most appropriate
  configuration for the current set of outputs and applies it. Useful
  if you updated a layout in your configuration file or want to set
  the configuration to the best matching configuration.
+ =output-layout-apply= :: Allows you to select the output layout to
  use. The selection list shown only contains the currently valid
  configurations.
