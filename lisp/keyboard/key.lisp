...
(defun pprint-key (key &optional (stream *standard-output*))
  "Print KEY in its abbreviated form (i.e. C-a, M-b, s-c, etc)."
  (declare (type key key) (type stream stream)
           (optimize (safety 1)))
  (let ((mod (key-modifier-mask key))
        (stumpwm-name (keysym->stumpwm-name (key-keysym key))))
    (when (not (= mod 0))
      (when (/= 0 (logand +modifier-ctrl+ mod))
        (format stream "C-"))
      (when (/= 0 (logand +modifier-alt+ mod))
        (format stream "M-"))
      (when (and (/= 0 (logand +modifier-shift+ mod))
                 (not (length= 1 stumpwm-name)))
        (format stream "S-"))
      ;; (when (/= 0 (logand +modifier-caps+ mod))
      ;;   (format stream "CAPS"))
      ;; FIXME: one of these modifiers is probably the hyper key
