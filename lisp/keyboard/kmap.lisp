...
Returns:
  ...
  (declare (type key-state bindings-state)
           (type key key)
           (optimize (speed 3) (safety 1)))
  (flet ((%lookup-values (func initial)
           (declare (type (function (t) t) func)
                    (type list initial)
                    (optimize (speed 3) (safety 1)))
           (multiple-value-bind (init-val rest)
	           (do* ((rest initial (cdr rest))
			         (cur (car rest) (car rest)))
		            ((not rest))
		         (let ((val (funcall func cur)))
		           (when val
			         (return (values val (cdr rest))))))
             (declare (type (or null list) rest))
             (when init-val
	           (do* ((lst (cons init-val nil))
		             (tail lst)
		             (rest rest (cdr rest))
		             (cur (car rest) (car rest)))
		            ((not rest) lst)
    ...
    (with-accessors ((kmaps key-state-kmaps)
        ...
        bindings-state
      (let* ((matching-values (%lookup-values
                               (lambda (m)
